...
contextBridge.exposeInMainWorld('api', {
  ...
  savePlan: (filePath: string, content: string) => ipcRenderer.invoke('save-plan', filePath, content),
  getStats: () => ipcRenderer.invoke('get-stats'),
  getMemories: () => ipcRenderer.invoke('get-memories'),
  readMemory: (filePath: string) => ipcRenderer.invoke('read-memory', filePath),
  getProjects: (showArchived: boolean) => ipcRenderer.invoke('get-projects', showArchived),
  ...
  renameSession: (id: string, name: string) => ipcRenderer.invoke('rename-session', id, name),
  archiveSession: (id: string, archived: boolean) => ipcRenderer.invoke('archive-session', id, archived),
  openTerminal: (id: string, projectPath: string, isNew: boolean, sessionOptions?: any) => ipcRenderer.invoke('open-terminal', id, projectPath, isNew, sessionOptions),
  search: (type: string, query: string) => ipcRenderer.invoke('search', type, query),
  readSessionJsonl: (sessionId: string) => ipcRenderer.invoke('read-session-jsonl', sessionId),

  // Settings
  getSetting: (key: string) => ipcRenderer.invoke('get-setting', key),
  setSetting: (key: string, value: any) => ipcRenderer.invoke('set-setting', key, value),
  deleteSetting: (key: string) => ipcRenderer.invoke('delete-setting', key),
  getEffectiveSettings: (projectPath: string) => ipcRenderer.invoke('get-effective-settings', projectPath),

  browseFolder: () => ipcRenderer.invoke('browse-folder'),
  addProject: (projectPath: string) => ipcRenderer.invoke('add-project', projectPath),
  removeProject: (projectPath: string) => ipcRenderer.invoke('remove-project', projectPath),
  ...
  onProcessExited: (callback: (sessionId: string, exitCode: number) => void) => {
    ipcRenderer.on('process-exited', (_event, sessionId, exitCode) => callback(sessionId, exitCode));
  },
  onProgressState: (callback: (sessionId: string, state: number, percent: number) => void) => {
    ipcRenderer.on('progress-state', (_event, sessionId, state, percent) => callback(sessionId, state, percent));
  },
  onTerminalNotification: (callback: (sessionId: string, message: string) => void) => {
